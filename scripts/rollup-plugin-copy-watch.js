    ...
    const doCopy = async () => {
        ...
        if (Array.isArray(targets) && targets.length) {
            for (const target of targets) {
                ...
                let { dest, rename, src, transform, ...restTargetOptions } = target;

                if (!src) {
                    throw new Error(`${stringify(target)} target must have "src" property`);
                }
                ...
                if (rename && typeof rename !== "string" && typeof rename !== "function") {
                    throw new Error(`${stringify(target)} target's "rename" property must be a \
string or a function`);
    ...
    return {
        name: "copy-watch",
        generateBundle: async (options_) => {
            outputDir = options_.dir || path.dirname(options_.file);
